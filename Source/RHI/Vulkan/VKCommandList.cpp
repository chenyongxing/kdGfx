...
namespace kdGfx
{
	...
	void VKCommandList::beginRenderPass(const RenderPassDesc& desc)
	{
		...
		{
			const RenderPassAttachment& attach = desc.colorAttachments[i];
			...
			auto& clearValue = attach.clearColor;
			attachmentInfo.clearValue.color = { clearValue[0], clearValue[1], clearValue[2], clearValue[3] };
		}

		VkRenderingAttachmentInfo depthAttachmentInfo = { VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO };

		auto& vkTexture0 = std::dynamic_pointer_cast<VKTextureView>(desc.colorAttachments[0].textureView)->getTexture();
		...
		{
			const RenderPassAttachment& attach = desc.depthAttachment;
			auto vkTextureView = std::dynamic_pointer_cast<VKTextureView>(attach.textureView);
			...
			depthAttachmentInfo.clearValue.depthStencil.depth = attach.clearDepth;
	...
	void VKCommandList::setVertexBuffer(uint32_t slot, const std::shared_ptr<Buffer>& buffer)
	{
	...
	void VKCommandList::setIndexBuffer(const std::shared_ptr<Buffer>& buffer, Format indexForamt)
	{
		auto vkBuffer = std::dynamic_pointer_cast<VKBuffer>(buffer);
		...
		vkCmdBindIndexBuffer(_commandBuffer, vkBuffer->getBuffer(), 0, indexType);
	}
	...
	void VKCommandList::copyBufferToTexture(const std::shared_ptr<Buffer>& buffer, const std::shared_ptr<Texture>& texture, uint32_t mipLevel)
	{
		...
		copyRegion.imageExtent = { texture->getWidth(), texture->getHeight(), 1 };
		...
		glm::uvec2 size, glm::ivec2 srcOffset, glm::ivec2 dstOffset)
	{
		...
		copyRegion.srcOffset = { srcOffset.x, srcOffset.y, 0 };
		copyRegion.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		copyRegion.dstSubresource.mipLevel = 0;
		copyRegion.dstSubresource.layerCount = 1;
		copyRegion.dstOffset = { dstOffset.x, dstOffset.y, 0 };
		copyRegion.extent = { size.x, size.y, 1 };
