...
namespace kdGfx
{
	...
	void DXCommandList::beginRenderPass(const RenderPassDesc& desc)
	{
		...
		for (size_t i = 0; i < renderTargetDescs.size(); i++)
		{
			...
			memcpy(renderTargetDesc.BeginningAccess.Clear.ClearValue.Color, attach.clearColor.data(), 4 * sizeof(float));
		...
		bool hasDepth = (desc.depthAttachment.textureView != nullptr);
		...
		if (hasDepth)
		{
			...
			depthStencilDesc.DepthBeginningAccess.Clear.ClearValue.DepthStencil.Depth = attach.clearDepth;
		...
		_commandList4->BeginRenderPass(renderTargetDescs.size(), renderTargetDescs.data(), 
			hasDepth ? &depthStencilDesc : nullptr, D3D12_RENDER_PASS_FLAG_NONE);
	}
	...
	void DXCommandList::setVertexBuffer(uint32_t slot, const std::shared_ptr<Buffer>& buffer)
	{
		auto dxBuffer = std::dynamic_pointer_cast<DXBuffer>(buffer);

		D3D12_VERTEX_BUFFER_VIEW vertexBufferView = {};
		vertexBufferView.BufferLocation = dxBuffer->getResource()->GetGPUVirtualAddress();
	...
	void DXCommandList::setIndexBuffer(const std::shared_ptr<Buffer>& buffer, Format indexForamt)
	{
		auto dxBuffer = std::dynamic_pointer_cast<DXBuffer>(buffer);
		D3D12_INDEX_BUFFER_VIEW indexBufferView = {};
		indexBufferView.BufferLocation = dxBuffer->getResource()->GetGPUVirtualAddress();
