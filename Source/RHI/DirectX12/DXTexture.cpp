		...
		_texture(texture)
	{
		...
		if (HasAnyBits(texture.getDesc().usage, TextureUsage::ColorAttachment))
		{
			...
			D3D12_RENDER_TARGET_VIEW_DESC rtvDesc =
			{
				...
				.ViewDimension = textureDesc.sampleCount > 1 ? D3D12_RTV_DIMENSION_TEXTURE2DMS : D3D12_RTV_DIMENSION_TEXTURE2D,
		...
		else if (HasAnyBits(texture.getDesc().usage, TextureUsage::DepthStencilAttachment))
		{
			...
			{
				.Format = format,
				.ViewDimension = textureDesc.sampleCount > 1 ? D3D12_DSV_DIMENSION_TEXTURE2DMS : D3D12_DSV_DIMENSION_TEXTURE2D,
