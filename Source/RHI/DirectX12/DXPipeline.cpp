...
namespace kdGfx
{
	...
	void DXRasterPipeline::_createPipelineState()
	{
		...
		if (_desc.colorWrites.size() < _desc.colorFormats.size())
		{
			_desc.colorWrites.resize(_desc.colorFormats.size());
		...
		for (uint32_t i = 0; i < _desc.colorFormats.size(); ++i)
		{
			...
			rtBlendDesc.RenderTargetWriteMask = _desc.colorWrites[i] ? D3D12_COLOR_WRITE_ENABLE_ALL : 0;
		}

		CD3DX12_DEPTH_STENCIL_DESC1 depthStencilDesc(D3D12_DEFAULT);
		depthStencilDesc.DepthEnable = _desc.depthTest.enable;
		depthStencilDesc.DepthWriteMask = (_desc.depthStencilWrite ? D3D12_DEPTH_WRITE_MASK_ALL : D3D12_DEPTH_WRITE_MASK_ZERO);
		switch (_desc.depthTest.compareOp)
		{
		...
		depthStencilDesc.StencilEnable = _desc.stencilTest.enable;
		depthStencilDesc.StencilReadMask = 0xFF;
		depthStencilDesc.StencilWriteMask = _desc.depthStencilWrite ? 0xFF : 0;
		switch (_desc.stencilTest.failOp)
		{
		case StencilOp::Keep:
		...
		switch (_desc.stencilTest.passOp)
		{
		case StencilOp::Keep:
		...
		switch (_desc.stencilTest.compareOp)
		{
		case CompareOp::Always:
